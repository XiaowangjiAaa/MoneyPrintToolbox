        ...
        {% if items %}
            ...
            <div class="detail-list" id="detailList">
                ...
                <div class="detail-row detail-item"
                    ...
                    <div>
                        <div class="num-box market-price">¥ {{ "%.2f"|format(item.price) }}</div>
                        <div class="small">当前市场价</div>
                    </div>

                    <div>
                        <div class="num-box cost-price">¥ {{ "%.2f"|format(item.purchase_price) }}</div>
                        <div class="small">购入成本价</div>
                    </div>

                    <div>
                        <div class="profit-value {{ 'profit-plus' if item.profit >= 0 else 'profit-minus' }}">
...
document.querySelectorAll(".cost-save-form").forEach(form => {
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const costBox = card ? card.querySelector(".cost-price") : null;
            const marketBox = card ? card.querySelector(".market-price") : null;
            const profitBox = card ? card.querySelector(".profit-value") : null;

            if (costBox && !Number.isNaN(v)) {
                costBox.textContent = "¥ " + v.toFixed(2);
            }
            if (card && !Number.isNaN(v)) {
                card.setAttribute("data-cost", String(v));
            }
            if (profitBox && marketBox && !Number.isNaN(v)) {
                const market = Number((marketBox.textContent || "").replace(/[^\\d.-]/g, ""));
                if (!Number.isNaN(market)) {
                    const profit = market - v;
                    profitBox.textContent = "¥ " + profit.toFixed(2);
                    profitBox.className = "profit-value " + (profit >= 0 ? "profit-plus" : "profit-minus");
                }
            }
            applyFilters();
        } catch (e) {
    ...
    {% if items %}
        ...
        <div class="list">
            ...
            <div class="row">
                ...
                <div class="num market-price">¥ {{ "%.2f"|format(item.price) }}</div>
                <div>
                    <div class="num cost-price">¥ {{ "%.2f"|format(item.purchase_price) }}</div>
...
document.querySelectorAll(".cost-save-form").forEach(form => {
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const costBox = row ? row.querySelector(".cost-price") : null;
            const marketBox = row ? row.querySelector(".market-price") : null;
            const profitBox = row ? row.querySelector(".profit-value") : null;
            const v = Number(priceInput ? priceInput.value : "0");
            if (costBox && !Number.isNaN(v)) {
                costBox.textContent = "¥ " + v.toFixed(2);
            }
